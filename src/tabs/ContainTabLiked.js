...
import { Text, StatusBar, Platform, Image, View, TouchableOpacity, FlatList, RefreshControl, BackHandler } from 'react-native';
import { Container, Body, Content, Header, Button, Left, Right } from 'native-base';
...
@observer
export default class ContainTabLiked extends Component {
  state = {
    dataSource: 0,
    refreshing: false,
    scrolled: false,
  ...
  constructor(props) {
    ...
    let liked = '';
    for (let i in store.liked) {
      liked += i + ',';
    }

    liked = liked.slice(0, -1);
  ...
  reset() {
    if (Object.keys(store.liked).length > 0)
      ...
      return (<Container style={styles.container}>
        <Header style={styles.searchBarContainer}>
          ...
          <Body style={styles.bodyAppBar}>
            <Text style={styles.headerTextOther}>{store.lang.liked}</Text>
          </Body>
        ...
        return (
          <Container style={styles.container}>
            ...
            <Content style={{ width: '100%' }}>
              <View style={styles.emptyBox}>
                <Text style={styles.emptyHeaderText}>{store.lang.likedempty}</Text>

                <Text style={styles.emptySubHeaderText}>
                  {
                    store.lang.likedemptySub
                  }
                </Text>
              </View>
            </Content>
